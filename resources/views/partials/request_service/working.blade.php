@if ($stage_working_color == 'info' || $stage_working_color == 'success')
    ...
    @if ($user_type != 'client' && $requestService->done_time == null)
...
<div class="message-box">
    <div class="messages">
        @foreach (\App\Models\Comment::with('user')->where('request_service_id', $requestService->id)->orderBy('created_at', 'desc')->get() as $comment)
            <div
                class="     
                            @if($comment->user && $comment->user->user_type == 'staff')
                                @if($user_type == 'client')
                                    incoming_msg_extra
                                @else 
                                    outgoing_msg_extra
                            ...
                            ">
                <div>
                    <b>{{ $comment->user ? $comment->user->name : '' }} @if($comment->user && $comment->user->user_type == 'staff') <small><b>(الادارة)</b></small> @endif</b>
                    <p>
